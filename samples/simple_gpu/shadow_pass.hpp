...
public:
    ...
    void Draw(const ShadowDrawSceneInfo& sceneInfo, GPUCommandBufferID cmd, const glm::mat4& view, const glm::mat4& proj, const glm::vec4& viewPos, const glm::vec3 lightPos)
    {
        ...
        memcpy(mUBO->cpu_mapped_address, &lightSpaceMatrix, sizeof(glm::mat4));
        ...
        tex_barrier.texture   = mDepthTexture;
        tex_barrier.src_state = GPU_RESOURCE_STATE_UNDEFINED;
        tex_barrier.dst_state = GPU_RESOURCE_STATE_DEPTH_WRITE;
        ...
        render_pass_desc.render_target_count = 0;
