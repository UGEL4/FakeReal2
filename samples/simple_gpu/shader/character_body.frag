...
void main()
{
    ...
    //mask
    vec3 mask_color = texture(sampler2D(texMask, texSamp), inUV).rgb;
    float S = mask_color.r;
    float metallic = mask_color.g;
    float multA = mask_color.r * 1.0;
    float multB = mask_color.r * 1.0;
    float roughness = (1.0 - mask_color.g) * multA + mask_color.g * multB;

    vec3 N = texture(sampler2D(texNormal, texSamp), inUV).rgb;
