#include "Backend/RenderGraphBackend.h"
...
namespace FakeReal
{
    namespace render_graph
    {
        ...
        RenderGraphBackend::RenderGraphBackend(const RenderGraphBuilder& builder)
            : m_pDevice(builder.m_pDevice)
            , m_pQueue(builder.m_pQueue)
        {
        }
        ...
        void RenderGraphBackend::ExecuteRenderPass(RenderPassNode* pass, RenderGraphFrameExecutor& executor)
        {
...

            {
                ...
                for (auto edge : writeEdges)
                {
                    ...
                    bool is_depth_stencil = FormatUtil_IsDepthStencilFormat((EGPUFormat)(Resolve(executor, *targetTexture)->format));
                    bool is_depth_only    = FormatUtil_IsDepthOnlyFormat((EGPUFormat)(Resolve(executor, *targetTexture)->format));
        ...
        GPUBindTableID RenderGraphBackend::AllocateAndUpdatePassBindTable(RenderGraphFrameExecutor& executor, PassNode* pass, GPURootSignatureID root_sig)
        {
            ...
            for (uint32_t i = 0; i < texReadEdges.size(); i++)
            {
                ...
                bool is_depth_stencil              = FormatUtil_IsDepthStencilFormat(view_desc.format);
                bool is_depth_only                 = FormatUtil_IsDepthOnlyFormat(view_desc.format);
